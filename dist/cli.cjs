...
// src/cli.ts
commander.program.name("doccrunch").description("Document ingestion engine: any doc \u2192 normalized JSON").version("0.1.0");
commander.program.command("parse <file>").description("Parse a document file and emit validated JSON").option(
  "--type <type>",
  "Force document type: merchant-statement | bank-csv | esb-meter"
).option("--pretty", "Pretty-print JSON output", false).action(async (file, opts) => {
  try {
    const content = await promises.readFile(file, "utf-8");
    const parseOpts = opts.type ? { type: opts.type } : void 0;
